...
import { Task, DesignQuestion, DesignInterview, DesignArtifacts, QuestionCategory, TestCase, TestPlan } from '../../shared/types'
import type { AgentId } from '../../shared/types'
import { Guard } from '../guard'

const CATEGORIES: Array<{ id: QuestionCategory; label: string; critical: boolean }> = [
...
const CATEGORY_TRIGGERS: Record<QuestionCategory, string[]> = {
...
export class Designer {
  private interview?: DesignInterview
  private agentId: AgentId = 'designer'
  ...
  async startInterview(featurePath: string, tasks: Task[]): Promise<DesignQuestion[]> {
  ...
  isReadyToFinalize(): { ready: boolean; missingCategories: string[] } {
    if (!this.interview) return { ready: false, missingCategories: [] }
    ...
    for (const cat of CATEGORIES) {
      ...
      const resolved = this.interview.questions.filter(
        (q) => q.category === cat.id && q.resolved
      )
  ...
  async finalize(): Promise<DesignArtifacts> {
    if (!this.interview) {
  ...
  async askQuestion(questionId: string): Promise<string> {
    if (!this.interview) return ''
    const q = this.interview.questions.find((q) => q.id === questionId)
    return q ? q.question : ''
  }
  ...
  interviewSummary(): string {
    if (!this.interview) return 'No interview active.'
    ...
    for (const cat of CATEGORIES) {
      ...
      if (total > 0) {
        ...
        lines.push(
          `- ${cat.label}: ${resolved}/${total} ${bar} ${cat.critical ? '[关键]' : ''}`
        )
      }
    }
    ...
    if (!status.ready) {
      lines.push('')
      lines.push(
        '### 缺失关键类别: ' + status.missingCategories.join(', ')
      )
    } else {
      lines.push('')
  ...
  goToNextRound(): DesignQuestion[] {
    ...
    if (remaining.length === 0) {
      for (const cat of CATEGORIES) {
        if (!cat.critical) continue
        const unanswered = this.interview.questions.some(
        (q) => q.category === cat.id && !q.resolved
      )
      const hasShortAnswers = this.interview.questions.some(
        (q) => q.category === cat.id && q.resolved && q.answer.length < 20
      )
  ...
  private generateInitialQuestions(
    ...
    const coveredCategories = new Set<QuestionCategory>()
    for (const cat of CATEGORIES) {
  ...
  private generateFollowUpQuestions(
    answered: DesignQuestion,
    answer: string,
  ): DesignQuestion[] {
    const followUps: DesignQuestion[] = []
  ...
  private generateDeepDiveQuestions(category: QuestionCategory): DesignQuestion[] {
    const questions: DesignQuestion[] = []
    let idSeed = Date.now() + 1000

    const deepDiveSeeds: Record<QuestionCategory, string[]> = {
  ...
  private generateTypesFromAnswers(): string {
    const resolved = this.getResolvedQuestions()
    ...
    if (!dataModelAnswers.trim()) return '// No type definitions generated — data model section not answered.'

    const lines: string[] = ['// Generated from design interview answers', '']

    const entityMatches = dataModelAnswers.matchAll(
      /(?:实体|Entity|Model|模型|struct|对象)[：:]\s*(\w+)/gi,
    )
    ...
    const fieldGroups = dataModelAnswers.matchAll(
      /(\w+)\s*(?:需要|包含|有|包含以下)\s*(?:字段|属性|field|property)[：:]\s*([\s\S]*?)(?=\.|\n\n|$)/gi,
    )
  ...
  private inferFieldType(field: string, context: string): string {
    const lower = field.toLowerCase()
    if (/id$|编号|标识/.test(lower)) return 'string'
    if (/时间|日期|created|updated|date|time|timestamp/.test(lower)) return 'string'
    if (/金额|价格|price|amount|数量|count|num|年龄|age/.test(lower)) return 'number'
    if (/是否|is_|has_|flag|enabled|disabled|状态|status/.test(lower)) return 'boolean'
    if (/列表|数组|list|array|items|集合/.test(lower)) return 'string[]'
    if (/对象|json|object|map|dict|config/.test(lower)) return 'Record<string, unknown>'
    return 'string'
  }
  ...
  private extractModulesFromAnswers(): string[] {
    const resolved = this.getResolvedQuestions()
    const modules: string[] = []
    const seen = new Set<string>()

    for (const q of resolved) {
      const moduleMatches = q.answer.matchAll(
        /(?:模块|Module|Service|服务|Controller|Handler|Repository)[：:]\s*(\w+)/gi,
      )
  ...
  private extractInterfacesFromAnswers(): string[] {
    const resolved = this.getResolvedQuestions()
    ...
    const methodMatches = interfaceAnswers.matchAll(
      /(?:方法|method|API|接口)[：:]\s*(\w+)/gi,
    )
  ...
  private collectEdgeCases(): string[] {
    const resolved = this.getResolvedQuestions()
  ...
  private buildDesignDoc(
    ...
    for (const cat of CATEGORIES) {
      const catQuestions = interview.questions.filter(
        (q) => q.category === cat.id && q.resolved,
      )
  ...
  private createTestCaseFromAC(id: string, ac: string): TestCase {
  ...
  private parseImplementationKeywords(content: string): Array<{ name: string; method?: string }> {
    const keywords: Array<{ name: string; method?: string }> = []

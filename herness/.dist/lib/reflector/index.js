...
class Reflector {
    ...
    guard;
    agentId = 'reflector';
    constructor(featuresDir, traceStore, guard) {
    ...
    async reflect(input) {
        ...
        const suggestedFixes = this.generateFixes(classified, input.featureName, input.designDoc, similarTraces);
    ...
    classifyErrors(errorDetails) {
        ...
        return errorDetails.map((detail) => {
            ...
            if (/is not assignable/i.test(lower) ||
            ...
            if (/cannot read propert/i.test(lower) ||
    ...
    diagnose(errors) {
        ...
        const lines = [`## Reflection Analysis`];
        lines.push('');
    ...
    generateFixes(errors, featureName, designDoc, similarTraces) {
        const fixes = [];
        const seen = new Set();
    ...
    shouldRetry(errors) {
        ...
        if (unknownCount > 0)
            return false;
        const highOrMedium = errors.filter(e => e.confidence === 'high' || e.confidence === 'medium').length;
        return highOrMedium > errors.length / 2;
    }
    extractLessons(errors, diagnosis) {
        if (!errors || errors.length === 0) {
        ...
        if (typeCounts.has('null_reference')) {
            lessons.push(`Guard against nullable values with optional chaining (?.) or null-checks before property access.`);
        }
        if (typeCounts.has('missing_import')) {
            lessons.push(`Ensure all dependencies are installed and imported before referencing them in code.`);
        }
        if (typeCounts.has('test_assertion')) {
            lessons.push(`Test assertions should be derived directly from acceptance criteria to avoid expectations that diverge from the design.`);
        }
        if (typeCounts.has('syntax_error')) {
            lessons.push(`Verify syntax after code generation — automated syntax errors suggest the template or generation logic needs review.`);
        }
        if (typeCounts.has('unknown')) {
            lessons.push(`Investigate uncategorized errors to improve classification rules for future reflection cycles.`);
        }
        if (lessons.length === 0) {
    ...
    deriveRootCause(errors, diagnosis) {
        if (!errors || errors.length === 0) {
    ...
    extractFilePath(errorDetail, featureName) {
        const pathPatterns = [

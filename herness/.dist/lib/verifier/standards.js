...
exports.StandardsVerifier = void 0;
const child_process_1 = require("child_process");
const fs_1 = require("fs");
const path_1 = require("path");
function runCommand(cwd, cmd, args) {
    try {
        (0, child_process_1.execSync)(`${cmd} ${args.join(' ')}`, { cwd, stdio: 'pipe', timeout: 60000 });
        ...
        for (const check of task.checks) {
            ...
            switch (check) {
                case 'compile': {
                    ...
                    if (!result.ok)
                        failures.push({ check, detail: result.output, severity: 'critical' });
                    break;
                }
                case 'typecheck': {
                    const tsconfig = (0, path_1.join)(task.featurePath, 'tsconfig.json');
                ...
                case 'lint': {
                    ...
                    const hasEslint = eslintNames.some(n => (0, fs_1.existsSync)((0, path_1.join)(task.featurePath, n)));
                    if (!hasEslint) {
                        checks[check] = 'skipped';
                        break;
                    }
                ...
                case 'design_coverage': {
                    ...
                    const srcDir = (0, path_1.join)(task.featurePath, 'src');
                    const missing = [];
                    for (const s of sections) {
                        const name = s.replace(/^##\s+/, '').trim();
                        if (name === '概述' || name === '任务列表' || name.startsWith('访谈'))
                            continue;
                        if (name === '数据模型' || name === 'Types') {
                            if (!(0, fs_1.existsSync)((0, path_1.join)(srcDir, 'types.ts')))
                                missing.push('数据模型/types.ts');
                        }
                        else if (name === '模块划分' || name === 'Modules') {
                            if (!(0, fs_1.existsSync)((0, path_1.join)(srcDir, 'index.ts')))
                                missing.push('模块划分');
                        }
                    }
                    checks[check] = missing.length === 0 ? 'passed' : 'failed';
                    if (missing.length > 0)
                        failures.push({ check, detail: `Missing: ${missing.join(', ')}`, severity: 'warning' });
                    break;
                }
                case 'code_style': {
                    const srcDir = (0, path_1.join)(task.featurePath, 'src');
                    ...
                    for (const file of walkTsFiles(srcDir)) {
                        ...
                        const fname = (0, path_1.basename)(file);
                        if (todos)
                            issues.push(`${fname}: ${todos.length} TODO(s)`);
                        if (anys)
                            issues.push(`${fname}: ${anys.length} 'any' type(s)`);
                    }
                    checks[check] = issues.length === 0 ? 'passed' : 'failed';
                    if (issues.length > 0)
                        failures.push({ check, detail: issues.join('; '), severity: 'warning' });
                    break;
                }
                default: {
                    checks[check] = 'skipped';
...
exports.StandardsVerifier = StandardsVerifier;

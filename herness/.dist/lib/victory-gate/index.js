...
const MINIMUM_BAR_CHECKS = {
    no_todo_in_src(srcDir) {
        const todos = [];
        if (!fs.existsSync(srcDir))
            return { satisfied: true, detail: 'no src dir to check' };
    ...
    all_imports_resolve(_srcDir) {
        return { satisfied: true, detail: 'verified by tsc --noEmit' };
    },
    coverage_file_exists(srcDir) {
...
class VictoryGate {
    ...
    retryCount = 0;
    previousVerdicts = [];
    constructor(featuresDir, config = DEFAULT_CONFIG) {
        this.featuresDir = featuresDir;
        this.config = config;
    }
    async evaluate(featureName, designArtifacts, validationReport, testPlan) {
        this.retryCount++;
        ...
        if (this.retryCount < this.config.maxRetries) {
            for (const issue of blockingIssues) {
                ...
                if (hint)
                    retryHints.push(hint);
            }
        }
        if (this.retryCount >= this.config.maxRetries) {
            blockingIssues.push(`Max retries (${this.config.maxRetries}) exceeded`);
            retryHints.push(`已达到最大重试次数 ${this.config.maxRetries}。建议手动审查 blockingIssues。`);
        }
        const totalChecks = acVerifications.length +
    ...
    verifyAcceptanceCriteria(featureContent, testPlan, featurePath) {
        ...
        return acLines.map((ac, index) => {
            const evidence = [];
            const gaps = [];
            const hasTestCoverage = testPlan?.functionalTests.some((t) => t.coverageTarget.includes(ac.slice(0, 20)) ||
                ac.toLowerCase().includes(t.description.toLowerCase().slice(0, 20))) ||
                testPlan?.unitTests.some((t) => t.coverageTarget.includes(ac.slice(0, 20)) ||
                    ac.toLowerCase().includes(t.description.toLowerCase().slice(0, 20)));
            if (hasTestCoverage) {
    ...
    checkDesignCoverage(artifacts, srcDir) {
        const sections = artifacts.designDoc.match(/^##\s+(.+)/gm);
        if (!sections)
            return { totalSections: 0, covered: 0, missing: [] };
    ...
    runCriticReview(featureContent, artifacts, srcDir) {
        const questions = [];
        let id = 0;
        for (const [category, patterns] of Object.entries(CRITIC_PATTERNS)) {
            for (const pattern of patterns) {
                id++;
                let assessment = 'passed';
                let detail = '';
                switch (category) {
                    case 'completeness':
                        detail = '检查中';
                    ...
                    case 'spec_gap': {
                        const unresolved = artifacts.interview?.questions?.filter((q) => !q.resolved);
    ...
    probeEdgeCases(featureContent, srcDir) {
        const cases = [];
    ...
    readFeatureDoc(featureName) {
        ...
        if (fs.existsSync(featurePath))
            return fs.readFileSync(featurePath, 'utf-8');
        return '';
    }
    readAllSrc(srcDir) {
        if (!fs.existsSync(srcDir))
    ...
    generateRetryHint(issue, _featureContent, _artifacts) {

...
const TYPE_DEFAULTS: Partial<Record<HernessUIType, Record<string, unknown>>> = {
  heading: { level: 1 },
  button: { variant: 'default' },
  input: { placeholder: '' },
}

export function validateScreen(screen: HernessScreen): DSLValidationResult {
...
function validateNode(node: HernessNode, path: string, errors: DSLValidationError[]): void {
...
export function normalizeScreen(screen: HernessScreen): HernessScreen {
  const children = screen.children.map(n => normalizeNode(n))
  return { ...screen, children }
}

function normalizeNode(node: HernessNode): HernessNode {
  const defaults = TYPE_DEFAULTS[node.type]

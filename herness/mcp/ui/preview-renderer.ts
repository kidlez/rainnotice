...
export function renderPreview(screen: HernessScreen): string {
...
function renderNode(node: HernessNode): string {
  ...
  switch (node.type) {
    ...
    case 'button': {
      const text = esc(node.props.text as string ?? '')
      const href = node.props.to as string
      if (href) return `<a href="${esc(href)}" class="h-btn ${css.classes}" style="${css.style}">${text}</a>`
      return `<button class="h-btn ${css.classes}" style="${css.style}">${text}</button>`
    }

    case 'input': {
      const placeholder = esc(node.props.placeholder as string ?? '')
      const name = esc(node.props.name as string ?? '')
    ...
    case 'checkbox': {
      const label = esc(node.props.label as string ?? '')
      const name = esc(node.props.name as string ?? '')
    ...
    case 'heading': {
      const level = Math.min(Math.max((node.props.level as number) ?? 2, 1), 6)
      const text = esc(node.props.text as string ?? '')
      return `<h${level} class="h-heading ${css.classes}" style="${css.style}">${text}</h${level}>`
    }

    case 'text': {
      const content = esc(node.props.content as string ?? '')
      return `<p class="h-text ${css.classes}" style="${css.style}">${content}</p>`
    }

    case 'paragraph':
      return `<p class="h-text ${css.classes}" style="${css.style}">${children || esc(node.props.content as string ?? '')}</p>`

    case 'label': {
      const text = esc(node.props.text as string ?? '')
      const htmlFor = node.props.for as string
      if (htmlFor) return `<label for="${esc(htmlFor)}" class="h-label ${css.classes}" style="${css.style}">${text}</label>`
      return `<span class="h-label ${css.classes}" style="${css.style}">${text}</span>`
    }

    case 'link': {
      const text = esc(node.props.text as string ?? '')
      const to = esc(node.props.to as string ?? '#')
      return `<a href="${to}" class="h-link ${css.classes}" style="${css.style}">${text}</a>`
    }

    case 'image': {
      const src = esc(node.props.src as string ?? '')
      const alt = esc(node.props.alt as string ?? '')
    ...
    case 'tooltip': {
      return `<span class="${css.classes}" style="position:relative;${css.style}">${children}<span class="h-tooltip-text" style="display:none;position:absolute;bottom:100%;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:4px 8px;border-radius:4px;font-size:12px;white-space:nowrap">${esc(node.props.text as string ?? '')}</span></span>`
    }
...
function buildCSS(node: HernessNode): { classes: string; style: string } {
  const classes: string[] = ['h-container']
  const styles: string[] = []

  if (node.layout) {
    const l = node.layout
    if (l.direction === 'row') { classes.push('h-row') }
    if (l.direction === 'column') { classes.push('h-col') }
    if (l.align) classes.push(`h-${l.align}`)
    if (l.justify) classes.push(`h-${l.justify}`)
    if (l.wrap) classes.push('h-wrap')
    if (l.gap && l.gap > 0) styles.push(`gap:${l.gap}px`)
    if (l.padding) {
      if (typeof l.padding === 'number') styles.push(`padding:${l.padding}px`)
      else styles.push(`padding:${l.padding.top}px ${l.padding.right}px ${l.padding.bottom}px ${l.padding.left}px`)
    }
  ...
  if (node.style) {
    ...
    if (s.maxWidth) styles.push(`max-width:${typeof s.maxWidth === 'number' ? s.maxWidth + 'px' : s.maxWidth}`)
    if (s.backgroundColor) styles.push(`background:${s.backgroundColor}`)
    if (s.borderColor) styles.push(`border:1px solid ${s.borderColor}`)
    if (s.borderRadius && s.borderRadius > 0) styles.push(`border-radius:${s.borderRadius}px`)
    if (s.borderWidth && s.borderWidth > 0) styles.push(`border-width:${s.borderWidth}px`)
    if (s.shadow) styles.push(`box-shadow:${s.shadow}`)
    ...
    if (s.margin) {
      if (typeof s.margin === 'number') styles.push(`margin:${s.margin}px`)
      else styles.push(`margin:${s.margin.top}px ${s.margin.right}px ${s.margin.bottom}px ${s.margin.left}px`)
    }
  }
...
function renderMenuItems(nodes: HernessNode[]): string {
  return nodes.map(n => {
    const text = esc(n.props.text as string ?? '')
    return `<li class="h-menu-item">${text}</li>`
  }).join('\n')
}

function renderTabs(nodes: HernessNode[]): string {
